...
auto args_from_lhs_type(const Type& lhs_type) -> std::vector<Type> {
  if (lhs_type.kind == TypeKind::kTuple) {
    return lhs_type.items;
  }
  return {lhs_type};
}
...
auto callable_has_fixed_arity(const Type& type, const std::size_t arity) -> bool {
  ...
  if (type.function_params.size() != arity) {
    return false;
  }
...
auto callable_accepts_arg(const Type& callable_type, const std::size_t param_index, const Type& arg_type) -> bool {
  if (is_deferred_callable_type(callable_type)) {
    return true;
  }
  if (callable_type.kind != TypeKind::kFunction) {
    return false;
  }
  if (!callable_type.function_return.has_value()) {
    return true;
  }
  if (param_index >= callable_type.function_params.size()) {
    return false;
  }
