...
auto usage_text() -> std::string { return "usage: fleaux2cpp [file.fleaux]"; }
...
auto parse_cli_args(int argc, char** argv) -> std::optional<CliOptions> {
  ...
  if (!options.source.has_value()) { options.source = std::filesystem::path("test.fleaux"); }
...
auto main(int argc, char** argv) -> int {
  ...
  const auto result = transpiler.process(*options->source);
