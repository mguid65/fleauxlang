...
inline auto parse_positive_size_option(const Object& options, const std::string_view key) -> std::optional<std::size_t> {
...
inline auto run_parallel_map_pooled(const Array& items, const Value& function_ref,
                                    const std::size_t max_in_flight) -> Value {
    if (items.Size() == 0) {
    ...
    while (!in_flight.empty()) {
        ...
        if (!ok_tag || !*ok_tag) {
            ...
            const auto msg = payload.HasArray() && as_array(payload).Size() == 1
                                 ...
                                 : to_string(payload);
            return ResultErr{}(make_tuple(
                make_tuple(make_int(static_cast<Int>(item_index)), make_string(msg))));
        }
...
struct TaskAwaitAll {
    ...
    auto operator()(Value arg) const -> Value {
        ...
        for (std::size_t task_index = 0; task_index < tasks.Size(); ++task_index) {
            ...
            if (!as_bool(*tuple.TryGet(0))) {
                return ResultErr{}(make_tuple(
                    make_tuple(make_int(static_cast<Int>(task_index)), *tuple.TryGet(1))));
            }
...
struct TaskWithTimeout {
    ...
    auto operator()(Value arg) const -> Value {
        ...
        if (timeout_ms < 0) {
            throw std::invalid_argument{"Task.WithTimeout: timeout_ms must be non-negative"};
        }
    ...
    auto operator()(Value arg) const -> Value {
        ...
        try {
            ...
            const std::size_t max_in_flight =
                requested_workers.value_or(pool_workers);
...
inline auto run_parallel_foreach_pooled(const Array& items, const Value& function_ref,
                                        const std::size_t max_in_flight) -> Value {
    if (items.Size() == 0) {
        return ResultOk{}(make_tuple(Value{Array{}}));
    }
    ...
    while (!in_flight.empty()) {
        ...
        if (!result_arr || result_arr->Size() < 1) {
            return ResultErr{}(make_tuple(
                make_tuple(make_int(static_cast<Int>(item_index)), make_string("Parallel.ForEach: unexpected result shape"))));
        }
...
inline auto run_parallel_reduce_chunked(const Array& items, const Value& init,
                                        const Value& function_ref,
    ...
    struct Chunk { std::size_t start; std::size_t end; };
    ...
    for (const auto& [start, end] : chunks) {
        ...
        auto chunk_task = task_runtime().submit_untracked(
            RuntimeCallable{[fn = std::move(fn_copy), items_arr = std::move(chunk_copy),
                             seed = std::move(seed_copy), base_idx = first_idx](const Value& /*ignored*/) mutable -> Value {
