...
## Purpose
This guide helps contributors identify exactly where to implement a feature in `core/`, which tests to update, and what parity checks to run before opening a pull request.
...
- CLI entry and mode routing: `core/src/cli/vm_main.cpp`
...
- Execution:
  - Interpreter: `core/src/vm/interpreter.cpp`
...
- `fleaux_vm_interpreter`: interpreter execution path
...
- `core/tests/vm_samples_tests.cpp` parity case
...
- `core/tests/vm_samples_tests.cpp` parity spot-check
...
- `core/src/vm/interpreter.cpp` (REPL symbolic-import policy)

Tests:
- `core/tests/vm_samples_tests.cpp` import parity
...
### 7) Runtime semantics and VM-interpreter parity
Touch:
- `core/src/vm/interpreter.cpp`
- `core/src/vm/runtime.cpp`
...
## Cross-cutting rules
- Keep interpreter and VM semantics aligned for imports and callable resolution.
- For import changes, verify both paths: `Interpreter::run_file` and `bytecode::load_linked_module`.
...
- Type system: `typecheck_tests` plus at least one parity case in `vm_samples_tests`
...
- Add tests in the owning suite and one parity-oriented suite
- Run focused tests for touched areas, then broader parity runs

## Useful commands
```bash
ctest --test-dir core/cmake-build-debug -N
ctest --test-dir core/cmake-build-debug -R fleaux_vm_parity_tests --output-on-failure
```

```bash
./core/cmake-build-debug/bin/fleaux --help
./core/cmake-build-debug/bin/fleaux --mode interpreter samples/01_hello_world.fleaux
./core/cmake-build-debug/bin/fleaux --mode vm samples/01_hello_world.fleaux
```
